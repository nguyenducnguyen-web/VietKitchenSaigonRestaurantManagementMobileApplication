...
import com.example.ungdungdatdoanonlinenhvietkitchensg.databinding.ActivityTrangThaiGiaoHangBinding
import com.example.ungdungdatdoanonlinenhvietkitchensg.databinding.DeliveryItemBinding
...
class TrangThaiGiaoHangActivity : AppCompatActivity() {
    private lateinit var binding: ActivityTrangThaiGiaoHangBinding
    private lateinit var appDatabase: AppDatabase
    private lateinit var adapter: GiaoHangAdapter

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityTrangThaiGiaoHangBinding.inflate(layoutInflater)
        setContentView(binding.root)

        appDatabase = AppDatabase.getDatabase(this)
        adapter = GiaoHangAdapter(emptyList()) { loadDeliveryOrders() }
    ...
    private fun loadDeliveryOrders() {
        CoroutineScope(Dispatchers.Main).launch {
            val donHangList = appDatabase.appDao().getDeliveryDonHang()
            adapter.updateData(donHangList)
        }
    }
}

class GiaoHangAdapter(private var donHangList: List<DonHang>, private val onOrderUpdated: () -> Unit) :
    RecyclerView.Adapter<GiaoHangAdapter.DonHangViewHolder>() {
    ...
    inner class DonHangViewHolder(private val binding: DeliveryItemBinding) :
        ...
        fun bind(donHang: DonHang) {
            CoroutineScope(Dispatchers.Main).launch {
                ...
                binding.btnDonHangDaChuanBiXong.setOnClickListener {
                    CoroutineScope(Dispatchers.Main).launch {
                        appDatabase.appDao().updateTrangThaiDonHang(donHang.id, "Đơn hàng đã chuẩn bị xong")
                        onOrderUpdated()
                        NotificationManager.addNotification(
                            "Đơn đặt hàng của bạn đã được chuẩn bị xong",
                            R.drawable.xacnhan
                        )
                    }
                }

                binding.btnDonHangDangTrenDuongGiaoDenBan.setOnClickListener {
                    CoroutineScope(Dispatchers.Main).launch {
                        appDatabase.appDao().updateTrangThaiDonHang(donHang.id, "Đơn hàng đang trên đường giao đến bạn")
                        onOrderUpdated()
                        NotificationManager.addNotification(
